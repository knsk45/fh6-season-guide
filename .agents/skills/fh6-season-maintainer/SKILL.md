...
Keep the public rolling history in `data/publication-metrics-history.json` (last 60 verified snapshots) and render its single native SVG trend graph inside the support block. The report must not load a graph library or retain personal completion data: card completion/filter state lives only in browser local storage. Never render copy buttons or clipboard controls for share codes. Card provenance labels must describe source status without claiming an unperformed in-game test.
