...
6. For a Thursday 20:30 preflight, run only `automation/season_rollover_preflight.ps1`; report failures but do not alter the season or publish.
...
2. For the combined Daily card, independently seek the complete list of all `season.expectedDailyItems` in the live official Playlist/News and fresh current-week Reddit Information Threads before relying on which days happen to be visible in a screenshot. Do not use a screenshot card or publish a visual tile for the Daily block: it is rendered as a text-only list of conditions and days. A screenshot may be retained only as private evidence for checking; do not wait for future Daily rows to open in it. Fill confirmed future days immediately, and request a condition screenshot only after the live sources have been exhausted.
3. Independently audit the `visual` of every activity on every run, including visuals not listed in `openItems`. Check the current Season page, official Playlist/News, fresh current-week Reddit guides, and current-week guide sites for a more exact image.
4. Prefer an exact current-week game tile. Exhaust the required live sources before deciding it is unavailable. If no current-season in-game tile can be found, do not promote a generic, prior-week, AI-generated, or season-overview fallback: leave the visual unresolved, add matching `missingFields`/`openItems`, and ask the user for unedited in-game screenshots showing each complete tile. Once supplied, retain the original screenshot as raw evidence, crop only the exact tile bounds into the appropriate prepared asset, and document the screenshot/crop in `reports/SOURCE_NOTES.md`; do not remove or alter in-tile content. Store the raw proof as `visual.sourceImage` and the prepared tile as `visual.image`; choose only `horizontal` or `vertical` for `visual.orientation`. Render the tile without overlays, darkening, a black square backing, or a crop that removes game content. On desktop, vertical tiles use the compact 280 px visual column with a reduced text-side inset; mobile keeps the tile above the text. Keep the card number beside its title. Put the optional activity icon beside the type/points line through `visual.typeIconKey`, not over the tile; use the matching reusable `data/project.json.activityIconLibrary` icon only when the tile has it, otherwise `null`. Horizon Play is the sole exception and always uses `horizon-play`. Do not redownload a library icon weekly. Icons must be a fixed compact size.
5. Research the remaining gaps plus fresh corrections for the active week.
6. Update `data/current-season.json`. Keep unknown values empty and update both `missingFields` and `openItems` together. Record the visual audit summary and counts by completeness status in `reports/SOURCE_NOTES.md`.
7. Run `automation/audit_visual_evidence.ps1` after the visual audit. Its local queue is the authoritative screenshot request list. If a tile is unresolved, omit it from the report rather than using a fallback.
8. After the complete required-source audit has succeeded and live evidence confirms the stored active season, run `automation/refresh_last_content_update.ps1`. `lastContentUpdate` is the time of the latest successful full audit, so refresh it even when facts, evidence, visuals, and recommendations did not change. If live confirmation or the required-source audit is incomplete, do not refresh it.
9. Run the full build, validation, and publication sequence below on every successful daily audit. A no-card-change run still publishes the refreshed verification time; it is not an empty commit.
...
3. Download exact compact current-season game tiles into a new directory under `reports/assets/`. Try to give every card its distinct current-week tile immediately. If an in-game tile cannot be found after the source audit, leave that visual unresolved and ask the user for an unedited screenshot of the complete tile; do not use a generic seasonal fallback. Preserve the submitted screenshot and crop only its tile into the appropriate horizontal or vertical asset after it arrives.
4. Run `pwsh -NoProfile -File automation/start_new_season.ps1 -InputPath automation/new-season-input.json`.
...
9. `automation/season_rollover_preflight.ps1` and `automation/audit_visual_evidence.ps1` are guarded steps; preflight validates operational readiness and the visual queue records unresolved exact tiles without blocking a truthful publication.
...
For Steam activities that require finding a specific place—Treasure Hunt, Photo Challenge, Collectibles, smashing objects, or similar location-based tasks—include one short direct link to the relevant location or map. Add it only when it materially helps locate the target; keep the condition text and activity order intact and do not add location links to activities without a concrete place.
...
Keep the public rolling history in `data/publication-metrics-history.json` (last 60 verified snapshots) and render its single native SVG trend graph inside the support block. The report must not load a graph library or retain personal completion data: card completion/filter state lives only in browser local storage. Visually highlight every nine-digit tuning share code in RU/EN web reports with semantic inline `<code>` markup. In Steam, display codes as plain selectable text formatted `NNN NNN NNN`; never wrap them in Steam `[code]` BBCode or code blocks because they render poorly there. Never add copy buttons or clipboard controls. Card provenance labels must describe source status without claiming an unperformed in-game test.
