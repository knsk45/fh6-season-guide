...
Неизвестное фактическое поле оставлять пустым. Для него обязательны статус `missing`/`preliminary`, имя в `missingFields` и запись в `openItems` с причиной и следующим источником для проверки. Для точной игровой плитки разрешён сезонный fallback-файл, но статус остаётся предварительным до появления нужной плитки. При каждом ежедневном запуске визуал каждой карточки перепроверяется независимо от `openItems`; отдельный актуальный скриншот активности всегда предпочтительнее общего fallback.
